...
class CrossEntropyCalculation(nn.Cell):
    ...
    def construct(self, logits, label_ids, num_labels):
        if self.is_training:
            label_ids=self.cast(label_ids,mstype.int32)
...
class FocalLossCalculation(nn.Cell):

    def __init__(self, weight=None, gamma=2.0, reduction='mean',is_training=None):
        super(FocalLossCalculation, self).__init__()
        # 校验gamma，这里的γ称作focusing parameter，γ>=0，称为调制系数
        self.focalloss=nn.FocalLoss(weight=weight,gamma=gamma,reduction=reduction)
    ...
    def construct(self, logits, label_ids, num_labels):
        if self.is_training:
            label_ids = self.reshape(label_ids, self.last_idx)
            one_hot_labels = self.onehot(label_ids, num_labels, self.on_value, self.off_value  )
            return_value=self.focalloss(logits=logits,labels=one_hot_labels)

        else:
            return_value = logits * 1.0
        return return_value

class CrossEntropyCalculationFinetune(nn.Cell):
    """
    Cross Entropy loss
    """
    def __init__(self, is_training=True,num_labels=2):
        super(CrossEntropyCalculationFinetune, self).__init__()
        self.onehot = P.OneHot()
        self.on_value = Tensor(1.0, mstype.float32)
        self.off_value = Tensor(0.0, mstype.float32)
        self.reduce_sum = P.ReduceSum()
...
class SSCalculation(nn.Cell):
    """
    Cross Entropy loss
    """
    def __init__(self, is_training=True):
        super(SSCalculation, self).__init__()
        self.onehot = P.OneHot()
        self.on_value = Tensor(1.0, mstype.float32)
        self.off_value = Tensor(0.0, mstype.float32)
        self.reduce_sum = P.ReduceSum()
    ...
    def step_end(self, run_context):
        ...
        if self._dataset_size > 0:
            ...
            if percent == 0:
                percent = 1
                epoch_num -= 1
            net_output=str(cb_params.net_outputs[0].asnumpy())
            print(f"***Epoch: {int(epoch_num)}***")
            print(f"Current epoch percent: {'%.3f' % percent}, step: {cb_params.cur_step_num}")
            print(f"Train loss: {net_output}")
        else:
