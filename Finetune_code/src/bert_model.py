...
class BertAttention(nn.Cell):
    ...
    def construct(self, from_tensor, to_tensor, attention_mask):
        ...
        query_out = self.query_layer(P.Cast()(from_tensor_2d,mindspore.float32))
        key_out = self.key_layer(P.Cast()(to_tensor_2d,mindspore.float32))
        value_out = self.value_layer(P.Cast()(to_tensor_2d,mindspore.float32))
...
class FinetuneSelfAttention(nn.Cell):
    """
    ...
    def __init__(self,
                 ...
                 use_relative_positions=False,
                 compute_type=mstype.float32):
        super(FinetuneSelfAttention, self).__init__()
...
class BertModel(nn.Cell):
    ...
    def construct(self, input_ids, token_type_ids, input_mask,return_all_encoders=False):
        """Bidirectional Encoder Representations from Transformers."""
        # embedding
        embedding_tables = self.bert_embedding_lookup.embedding_table
        word_embeddings = self.bert_embedding_lookup(P.Cast()(input_ids, mindspore.int32))
        ...
        if return_all_encoders==False:
            return sequence_output, pooled_output, embedding_tables
        else:
            all_sequence_output = []
            all_polled_output = []
            for idx_index in range(self.last_idx + 1):
                one_sequence_output = (self.cast(encoder_output[idx_index], self.dtype))
                all_sequence_output.append(one_sequence_output)
                one_sequence_slice = self.slice(one_sequence_output,
                                                (0, 0, 0),
                                                (batch_size, 1, self.hidden_size),
                                                (1, 1, 1))

                one_first_token = self.squeeze_1(one_sequence_slice)
                one_pooled_output = self.dense(one_first_token)
                one_pooled_output = self.cast(one_pooled_output, self.dtype)
                all_polled_output.append(one_pooled_output)

            return sequence_output, pooled_output, embedding_tables, all_sequence_output, all_polled_output




class BertModelAllSeqs(nn.Cell):
    """
    Bidirectional Encoder Representations from Transformers.
    ...
    def __init__(self,
                 ...
                 is_training,
                 use_one_hot_embeddings=False):
        super(BertModelAllSeqs, self).__init__()
        config = copy.deepcopy(config)
        if not is_training:
            config.hidden_dropout_prob = 0.0
            config.attention_probs_dropout_prob = 0.0
    ...
    def construct(self, input_ids, token_type_ids, input_mask):
        ...
        sequence_output = self.cast(encoder_output[self.last_idx], self.dtype)
        all_sequence_output = self.cast(encoder_output[self.last_idx], self.dtype)
        ...
        # get all layers
        for idx_index in range(3,self.last_idx):
            one_sequence_output = (self.cast(encoder_output[idx_index], self.dtype))
            all_sequence_output=all_sequence_output+one_sequence_output

        return all_sequence_output, pooled_output, embedding_tables


class BertModelEval(nn.Cell):
    """
    Bidirectional Encoder Representations from Transformers.
    ...
    def __init__(self,
                 ...
                 is_training,
                 use_one_hot_embeddings=False):
        super(BertModelEval, self).__init__()
        config = copy.deepcopy(config)
        if not is_training:
            config.hidden_dropout_prob = 0.0
            config.attention_probs_dropout_prob = 0.0
    ...
    def construct(self, input_ids, token_type_ids, input_mask):
        ...
        all_sequence_output=[]
        for idx_index in range(self.last_idx+1):
            all_sequence_output.append(self.cast(encoder_output[idx_index], self.dtype))
