...
def create_seq_dataset(batch_size=1, assessment_method="accuracy", data_file_path=None,
...
def create_seq_pd_dataset(batch_size=1, data_file_path=None,
                       dataset_format="mindrecord", schema_file_path=None, do_shuffle=True, drop_remainder=True):
    """create finetune or evaluation dataset"""
    type_cast_op = C.TypeCast(mstype.int32)
    type_cast_op_float = C.TypeCast(mstype.float32)
    if dataset_format == "mindrecord":
        dataset = ds.MindDataset([data_file_path],
                                 columns_list=["input_ids", "input_mask", "segment_ids","pssms","dssps", "label_ids"],
                                 shuffle=do_shuffle)
    elif dataset_format == "tfrecord":
        dataset = ds.TFRecordDataset([data_file_path], schema_file_path if schema_file_path != "" else None,
                                     columns_list=["input_ids", "input_mask", "segment_ids","pssms","dssps", "label_ids"],
    ...
    dataset = dataset.map(operations=type_cast_op, input_columns="input_mask")
    dataset = dataset.map(operations=type_cast_op, input_columns="input_ids")
    dataset = dataset.map(operations=type_cast_op_float, input_columns="pssms")
    dataset = dataset.map(operations=type_cast_op_float, input_columns="dssps")
...
def create_regress_dataset(batch_size=1, assessment_method="accuracy", data_file_path=None,
                                  schema_file_path=None, dataset_format="mindrecord", do_shuffle=True):
    """create finetune or evaluation dataset"""
    type_cast_op = C.TypeCast(mstype.int32)
    type_cast_fl = C.TypeCast(mstype.float32)

    data_set = ds.MindDataset([data_file_path],
                                  columns_list=["input_ids", "input_mask", "segment_ids", "label_ids"],
                                  shuffle=do_shuffle)

    data_set = data_set.map(operations=type_cast_fl, input_columns="label_ids")
    data_set = data_set.map(operations=type_cast_op, input_columns="segment_ids")
    data_set = data_set.map(operations=type_cast_op, input_columns="input_mask")
    data_set = data_set.map(operations=type_cast_op, input_columns="input_ids")
    # apply batch operations
