...
# from generate_pretrain_mindrecord_thread import thread_mindrecord
import collections
import random
from argparse import ArgumentParser
import tokenization
...
def create_training_instances(input_file, tokenizer, max_seq_length,
    ...
    print("Collect "+str(len(seqs)))

    for seq in seqs:
        tokens = tokenizer.tokenize(" ".join(list(seq)))
        if tokens:
            all_documents[-1].append(tokens)
...
def thread_mindrecord(input_file,output_file,vocab_file):
    # args=parse_args()
...
# file_indexs=[list(range(i*20,(i+1)*20)) for i in range(27)]
# file_indexs.append(list(range(540,542)))

file_indexs=[list(range(i,i+10)) for i in [390,400,410,420,430,440,450,460,470,480,490,500,510,520,530]]  #15
file_indexs.append([384,385,386,387,388,389,540,541])

mr_files=os.path.join(args.output_file,"mr_files_2")
...
for file in file_indexs[args.file_index]:
    file_name=os.path.join(fasta_files,"uniref50_"+str(file)+".fasta")
    print("run "+"uniref50_"+str(file)+".fasta")
    thread_mindrecord(file_name,os.path.join(mr_files,file_name.split("/")[-1].strip(".fasta")+".mindrecord"),args.vocab_file)
    print("finish "+"uniref50_"+str(file)+".fasta")
