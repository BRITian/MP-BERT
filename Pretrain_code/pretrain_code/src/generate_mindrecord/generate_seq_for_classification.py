...
def parse_args():
    ...
    parser.add_argument("--data_dir", type=str, default="/data1/bert/PMT/pmt_seqfaslenc04clstr/")
    parser.add_argument("--task_name", type=str, default="tnews", help="The name of the task to train.")
    parser.add_argument("--vocab_file", type=str, default="/data2/bert/mindspore/datas/uniref50/vocab.txt",
                        help="The vocabulary file that the BERT model was trained on.")
    parser.add_argument("--output_dir", type=str, default="/data2/bert/mindspore/datas/pmt_040/",
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
    ...
    tokens_a = tokenizer.tokenize(" ".join(list(example.text_a)))
    tokens_b = None
    if example.text_b:
...
class TnewsProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        return self._create_examples(
            self._read_csv(os.path.join(data_dir, "train_val_0.csv")), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_csv(os.path.join(data_dir, "val_0.csv")), "dev")

    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_csv(os.path.join(data_dir, "test_0.csv")), "test")
    ...
    def _create_examples(self, lines, set_type):
        ...
        for i in range(len(lines)):
            guid = "%s-%s" % (set_type, i)
            text_a = convert_to_unicode(lines['seq'][i])
            text_b = None
            label = convert_to_unicode(str(lines['label'][i]))
            examples.append(
