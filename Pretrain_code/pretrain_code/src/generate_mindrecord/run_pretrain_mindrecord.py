...
for file in file_indexs[args.file_index]:
    ...
    thread1 = threading.Thread(target = thread_mindrecord,args=(file_name,os.path.join(mr_files,file_name.split("/")[-1].strip(".fasta")+".mindrecord"),args.vocab_file))
    tsk_name.append(file_name.split("/")[-1])
    tsk_small.append(thread1)
    if len(tsk_small)==5:
        tsk.append(tsk_small)
        tsk_small=[]
if len(tsk_small)!=0:
    tsk.append(tsk_small)
...
i=0
j=0
for tsk_small in tqdm(range(len(tsk))):
    for t in tsk_small:
        print('start Thread ' +tsk_name[i])
        t.start()
        i+=1
