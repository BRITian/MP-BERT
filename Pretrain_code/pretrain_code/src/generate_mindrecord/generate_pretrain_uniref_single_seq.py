...
def create_training_instances(input_file, tokenizer, max_seq_length,
    ...
    print("Pfam number: "+str(count_files))
    print("File number "+str(len(all_documents)))
    ...
    for instances_iter in range(dupe_factor):
        ...
        write_instance_to_example_files(instances, tokenizer, args.max_seq_length,
                                        args.max_predictions_per_seq, args.output_file+"_fasta_index_"+str(args.index_num)+"_iter_"+str(instances_iter)+".mindrecord")
        instances=[]
    return instances
